...
#include "./../include/ResultsShow.h"
#include "./../include/CompilerPhases.h"
#include <stdlib.h>

#define LEXIC_PATH "./lexic-analyzer "
...
IDE_CompiladorFrame::IDE_CompiladorFrame(wxWindow* parent,wxWindowID id): wxFrame(parent, id, "Compilador ISC 8�", wxDefaultPosition, wxSize(800, 600))
{
...
void IDE_CompiladorFrame::OnCompile(wxCommandEvent& event){
    wxString Command(GetCurrentWorkingDirectory());
    Command.Append("/lexic-analyzer ");
    ...
    process->Redirect();
    long Code = wxExecute(Command, wxEXEC_SYNC, process);
    process->SetPid(Code);

    wxTextInputStream OutputStream(*process->GetInputStream());
    wxTextInputStream ErrorStream(*process->GetErrorStream());

    wxString Result = OutputStream.ReadLine();
    while(!Result.IsSameAs("ARBOL GRAMATICAL")){
        OutTokenString.Append(Result);
        OutTokenString.Append("\n");
        Result = OutputStream.ReadLine();
    }

    Result = OutputStream.ReadLine();
    while(!Result.IsSameAs("ARBOL SINTACTICO")){
        OutGrammarArray.Add(Result);
        Result = OutputStream.ReadLine();
    }
    ...
    wxString ErrResult = ErrorStream.ReadLine();
    while(!ErrResult.IsEmpty()){
        ErrorsString.Append(ErrResult);
        ErrorsString.Append('\n');
        ErrResult = ErrorStream.ReadLine();
    }
