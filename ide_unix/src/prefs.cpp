...
const char* CppWordlist1 =
    ...
    "export extern false float for friend goto if fi inline int long "
    "mutable namespace new operator private protected public register "
    "reinterpret_cast return short signed sizeof static static_cast "
    "struct switch template this throw true try typedef typeid "
    "typename union unsigned using virtual void volatile wchar_t "
    "while until then program write read and not or ";
...
const char* CppWordlist3 =
    ...
    "overload p page par param post pre ref relates remarks return read write "
    "retval sa section see showinitializer since skip skipline struct "
    "subsection test throw todo typedef union until var verbatim write read "
    "verbinclude version warning weakgroup and not or $ @ \"\" & < > # { }";
...
const LanguageInfo gLanguagePrefs [] = {
    // C++
    {"C++",
     "*.c;*.cc;*.cpp;*.cxx;*.cs;*.h;*.hh;*.hpp;*.hxx;*.sma;*.tny",
     wxSTC_LEX_CPP,
...
const int gStylePrefsSize = WXSIZEOF(gStylePrefs);
      ...
      wxString GetCurrentWorkingDirectory(){
          char ReadLinkFormat[30];
          char Buffer[PATH_MAXLEN];
          unsigned int CurrentPosition;
          sprintf(ReadLinkFormat, "/proc/%d/exe", (int)getpid());
          int BufferLength = readlink(ReadLinkFormat, Buffer, PATH_MAXLEN);
          if(BufferLength < 0){
              exit(EXIT_FAILURE);
          }

          Buffer[BufferLength] = '\0';
          for(CurrentPosition = strlen(Buffer)-1; CurrentPosition > 0; --CurrentPosition){
              if(Buffer[CurrentPosition] == '/'){
                  Buffer[CurrentPosition] = '\0';
                  break;
              }
          }
        wxString Result = wxString::FromAscii(Buffer);
        return Result;
      }
#else
        wxString GetCurrentWorkingDirectory(){
            wxString Result = wxGetCwd();
            return Result;
        }
#endif